...
> MediaTrace 只展示网页已经访问的媒体地址，不绕过 DRM、登录或网站访问控制。
...
- DRM、Blob/MSE、加密授权及短时效签名媒体不保证能够使用。
...
- 未经明确书面授权，不得将本项目或其修改版本用于任何直接或间接商业用途；
- 未经明确书面授权，不得将本项目的代码、文档、图片、构建产物、测试数据或修改版本用于人工智能或机器学习的数据训练。
...
如需商业使用、商业集成或无法确定某种使用方式是否属于商业用途，请在使用前取得项目权利人的书面授权。

## 更多文档

- [开发、构建与实现说明](docs/DEVELOPMENT.md)
- [Chrome Native Host 安装脚本](scripts/install-chrome-native-host.sh)
- [Safari 构建脚本](scripts/build-safari.sh)
